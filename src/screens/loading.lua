...
function LoadingScreen.new()
    ...
    function self:init()
        ...
        self.token = raw:match("^%s*(.-)%s*$")  -- trim whitespace/newlines

        self.status    = "connecting"   -- connecting | authing | success | failed | timeout
    ...
    function self:connectToServer()
        ...
        self.client:on("connect", function()
            ...
            self.client:send("reconnect_with_token", { token = self.token })
        end)
